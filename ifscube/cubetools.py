...
def wlprojection(arr, wl, wl0, fwhm=10, filtertype='box'):
	    ...
	    ind_non_zero = np.flatnonzero(
	                                  np.array(
                                          (wl >= lower_wl) & (wl <= upper_wl),
                                          dtype='float')
					  )
	    ind_non_zero = np.pad(ind_non_zero, (1,1),
	                         'constant',
		                 constant_values = (
	    ...
	    arrfilt /= trapz(arrfilt, wl, axis=0)                          # AINDA PRECISA? #
...
class gmosdc:
	...
	if angScale is None:
		...
		pScale = abs(self.header['CD1_1'])
	        ...
	        print ('WARNING! Angular scale \'CD1_1\' not found in the image' +
	...
	for i in np.arange(channels):
	    ...
	    if i==0:
		coords = self.peak_coords(wl_center=lambda0, wl_width=cw, center_type='peak_cen')
		...
		if (i==scale_panel):
		    ...
		    delt_y = (y.max() - y.min()) * 0.05
			    ...
			    path_effects=[path_effects.Stroke(linewidth=4.5,
			                                      foreground=stroke_color,
					                      alpha=0.3),
					  path_effects.Normal()])
        	    ax.annotate(
		    		scale_text, xy=(pos_x+scale_size/2., pos_y + delt_y),
		                ha='center',
				va='bottom',
				color=text_color,
				weight='bold',
				path_effects=[path_effects.Stroke(linewidth=1.5,
				                                  foreground=stroke_color,
								  alpha=0.3),
				              path_effects.Normal()])
		...
		if (i==n_panel):
		    ...
		    plt.plot(
		    	     [x_north, arrow_pos[1], x_east], [y_north, arrow_pos[0], y_east], '-',
		             lw=3, color=text_color,
			     path_effects=[path_effects.Stroke(linewidth=4.5,
			                                       foreground=stroke_color,
							       alpha=0.3),
					   path_effects.Normal()])
		    l_ang = -15
		    y_letter_n = arrow_pos[0] + 1.15*arrSize*np.sin(np.deg2rad(north_pa  +  l_ang))
		    x_letter_n = arrow_pos[1] + 1.15*arrSize*np.cos(np.deg2rad(north_pa  +  l_ang))
		    y_letter_e = arrow_pos[0] + 1.15*arrSize*np.sin(np.deg2rad(north_pa+90.+l_ang))
		    x_letter_e = arrow_pos[1] + 1.15*arrSize*np.cos(np.deg2rad(north_pa+90.+l_ang))
		    plt.text(
            ...
            ax.annotate(
                ...
                color=text_color, path_effects=[path_effects.Stroke(linewidth=1.5, foreground=stroke_color, alpha=0.3), path_effects.Normal()])
            if i % side != 0:
                ax.set_yticklabels([])
	...
	if colorBar:
	    ...
	    ax1 = fig.add_axes([x_max_axes_1, y_min_axes_0, 0.015, y_max_axes_1-y_min_axes_0]) # setup colorbar axes.
    ...
    def peak_coords(self, wl_center, wl_width, center_type='peak_cen',  spatial_center=[], spatial_width=10):
	"""
	...
	----------
		...
		peak and peak_cen.
		Where,
		'peak_cen' returns the centroid on a box 'spatial_width' wide, centered
		on the pixel corresponding to the peak value,
	...
	"""
	if (spatial_center == []):
		spatial_center=[int(self.data.shape[1]/2.),int(self.data.shape[2]/2.)]

	wavelenght = self.wl
	projection = wlprojection(arr=self.data,
	                          wl=self.wl,
				  wl0=wl_center,
				  fwhm=wl_width,
				  filtertype='box')
	...
	if (center_type=='peak_cen'):
		...
		spatial_center[0] = int(spatial_center[0] - spatial_width/2 + coords[0])
		spatial_center[1] = int(spatial_center[1] - spatial_width/2 + coords[1])
		projection_crop = projection[
	                        int(spatial_center[0]-spatial_width/2):
	                        int(spatial_center[0]+spatial_width/2)+1,
	                        int(spatial_center[1]-spatial_width/2):
    ...
    def rgb_line_compose(self, lambdas, velmin, velmax, channels=9,
	    continuum_width=300, continuum_opts=None, logFlux=True,
	    angScale=None, scaleBar={}, northArrow={}, lowerThreshold=1e-16,
	    plot_opts={}, fig_opts={}, wspace=None, hspace=None,
	    text_color='black', stroke_color='white', center_mark=True,
	...
	for i in np.arange(channels):
	    ...
	    if i==0:
		coords = self.peak_coords(wl_center=lambdas[0], wl_width=cw, center_type='peak_cen')
		...
		channel_b = (channel_b - chan_min_b) / (chan_max_b - chan_min_b)
	    elif (len(lambdas) == 2):
	    ...
	    channel = np.ma.dstack((channel_r, channel_g, channel_b, channel_alpha))
	    ax.set_facecolor(bg_color)
	    imap = ax.imshow(channel, extent=[x.min(), x.max(), y.min(), y.max()], origin='lower')
	    ax.set_aspect('equal', 'datalim')
	    if center_mark:
		mpl.pyplot.plot(0, 0, 'w+', lw=3)
		mpl.pyplot.plot(0, 0, 'k+', lw=2)
		...
		if (i==n_panel):
            ...
            ax.annotate(
                '{:.0f}'.format((wlc_r - lambdas[0])/lambdas[0]*2.99792e+5),
                xy=(0.1, 0.8), xycoords='axes fraction', weight='bold',
                color=text_color, path_effects=[path_effects.Stroke(linewidth=1.5, foreground=stroke_color, alpha=0.3), path_effects.Normal()])
            if i % side != 0:
                ax.set_yticklabels([])
