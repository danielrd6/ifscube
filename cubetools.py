...
class gmosdc:
  ...
  def __init__(self,fitsfile,redshift=0.0):
    """
...
#    self.fullwl = st.get_wl(fitsfile,hdrext=dataext,dimension=0,dwlkey='CD3_3',wl0key='CRVAL3',pix0key='CRPIX3')
#    self.fullrestwl = self.fullwl/(1.+redshift)
#
#    clipcond = (self.fullrestwl > wlclip[0])&(self.fullrestwl < wlclip[1])
#    self.wl = self.fullwl[clipcond]
#    self.restwl = self.fullrestwl[clipcond]

    self.wl = st.get_wl(fitsfile,hdrext=dataext,dimension=0,dwlkey='CD3_3',wl0key='CRVAL3',pix0key='CRPIX3')
  ...
  def continuum(self,niterate=3,degr=5,upper_threshold=1,lower_threshold=1,writefits=False,outimage=None):
    """
    ...
    for i in arange(shape(self.data)[1]):
      for j in arange(shape(self.data)[2]):
        s = self.data[:,i,j]
        if any(s[:20]):
          try:
            c[:,i,j] = st.continuum(self.restwl,s,niterate=niterate,degr=degr,upper_threshold=upper_threshold,lower_threshold=lower_threshold,returns='function')[1]
          except TypeError:
  ...
  def snr_eval(self,wl_range=[6050,6200]):
    ...
    for i in arange(shape(noise)[0]):
      for j in arange(shape(noise)[1]):
  ...
  def wlprojection(self,wl0,fwhm=10,filtertype='box',writefits=False,outimage='wlprojection.fits'):
    """
    ...
    if filtertype == 'box':
       arrfilt = array((self.restwl >= wl0-fwhm/2.)&(self.restwl <= wl0+fwhm/2.),dtype='float')
      ...
      print 'ERROR! Parameter filtertype "{:s}" not understood.'.format(filtertype)

    outim = zeros(shape(self.data)[1:])

    for i in arange(shape(outim)[0]):
      for j in arange(shape(outim)[1]):
  ...
  def haniisiifit(self, function='gaussian', p0=[50, 1e-16, 1e-16, 
                  1e-16, 1e-16, 1e-16, 2], fit_window=[6400,7000], 
                  writefits=False, outimage=None, trysmooth=False):
    """
    Fits the three emission lines centred in H alpha (6564 A) a given 
    function and returns a map of measured properties.
...
#    lc = array([6549.85, 6564.61, 6585.28])
  ...
  def voronoi_binning(self, targetsnr=10.0, writefits=False,
    ...
    if writefits:
      ...
      pf.writeto(outfile,data=self.binned,header=hdr,clobber=clobber)
